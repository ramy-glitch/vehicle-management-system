...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(empty($report_date) || $report_date=="0000-00-00" || !preg_match('/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/', $report_date)){  // Matches yyyy-mm-dd
    ...
    if (empty($errorMessages)) {
        ...
        $sql = "INSERT INTO driver_report (driver_id,report_date, report_issue, report_description) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($link, $sql);
        $stmt->bind_param("isss", $_SESSION['driver_id'] , $report_date, $report_title, $report_content);
        $stmt->execute();
        $stmt->close();

        // Redirect after successful submission
        header("Location: ddriverReports.php");
        exit;
    }
}
?>
